...
class CHDataWriter:
    ...
    async def _flush_participants(self, client: TelegramClient, group_id, user_id_to_desc):
        if len(user_id_to_desc) == 0:
            return
        users_csv = ','.join([str(id) for id in user_id_to_desc.keys()])
        ...
        peers = list()
        for user_id, desc in user_id_to_desc.items():
            if user_id not in known_users:
                peers.append(desc)
        resp = await client.get_entity(peers)
        ...
        async for message in client.iter_messages(target_group, limit=None, reverse=True,
            ...
            if message.from_id is not None and isinstance(message.from_id, PeerUser):
                ...
                if isinstance(peer, PeerUser):
                    user_id_to_desc[peer.user_id] = peer
